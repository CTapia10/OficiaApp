...
- **Last closed:** Sprint Hexagonal FE + Testing rules (`.cursorrules` §7). Backend hexagonal audit OK. Open debt: CI + Vitest.
- **Next:** Sprint 18 — Requests (Frontend): `POST /api/job-requests` + client's own list. Install Vitest in same sprint if logic lands (or micro-sprint first per §5).
...
- **Tests FE:** Vitest not installed — §5. Official stack when present: Vitest + Testing Library (`.cursorrules` §7.3).
...
- Sprint 18: Requests (Frontend) — `POST /api/job-requests` + client's own list; wire `RequestsView` (align with PRODUCT_MAP). Include FE tests once Vitest is available (install Vitest in same sprint if logic lands).
- **Vitest setup** (with Sprint 18 or micro-sprint first): Vitest + Testing Library; clear §5 debt.
...
- **CI:** `.github/workflows` empty — no automated `dotnet test` on PR/push.
- **Vitest:** Frontend test runner not installed yet — required before/with next FE feature that adds testable logic (`.cursorrules` §7.3).
